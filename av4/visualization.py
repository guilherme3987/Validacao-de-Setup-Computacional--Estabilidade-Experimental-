...
def plot_ops_vs_n(resultados, output_dir="resultados/imagens"):
    os.makedirs(output_dir, exist_ok=True)
    ...
    for nome_alg, dados in resultados.items():
        Ns_arr = np.array(dados["Ns"], dtype=float)
        O_arr  = np.array(dados["ops"], dtype=float)
        cor     = CORES_ALG.get(nome_alg, "#FFFFFF")
        marcador = MARCADORES_ALG.get(nome_alg, "o")

        if len(Ns_arr) >= 4:
            N_smooth = np.linspace(Ns_arr.min(), Ns_arr.max(), 500)
            spline   = make_interp_spline(Ns_arr, O_arr, k=3)
            O_smooth = spline(N_smooth)
            ax.plot(N_smooth, O_smooth, color=cor, lw=2.2, alpha=0.85)
        else:
            ax.plot(Ns_arr, O_arr, color=cor, lw=2.2, alpha=0.85)

        ax.scatter(Ns_arr, O_arr, color=cor, s=110, zorder=10,
                   edgecolors="#FFFFFF", linewidths=1.5,
                   marker=marcador, label=nome_alg)

    ax.set_title("Operações Totais vs N: Quicksort vs Shell Sort",
                 color="#F0F0F0", fontsize=15, fontweight="bold", pad=15)
    ax.set_xlabel("Tamanho da lista (N)", color="#AAAAAA", fontsize=12)
    ax.set_ylabel("Operações totais", color="#AAAAAA", fontsize=12)
    ax.tick_params(colors="#AAAAAA")
    for spine in ax.spines.values():
        spine.set_edgecolor("#333355")
    ...
    output_path = os.path.join(output_dir, "operacoes_vs_n.png")
    plt.savefig(output_path, dpi=160, bbox_inches="tight", facecolor=fig.get_facecolor())
    plt.close()
    print(f"Grafico salvo em {output_path}")


def plot_comparacao_normalizada(resultados, output_dir="resultados/imagens"):
    os.makedirs(output_dir, exist_ok=True)
    ...
    for nome_alg, dados in resultados.items():
        ...
        if len(Ns_arr) >= 4:
            N_smooth = np.linspace(Ns_arr.min(), Ns_arr.max(), 500)
            spline_o = make_interp_spline(Ns_arr, O_norm, k=3)
            spline_t = make_interp_spline(Ns_arr, T_norm, k=3)
            ax.plot(N_smooth, spline_o(N_smooth), color=cor_ops, lw=2.5, alpha=0.9, label="Operacoes (normalizado)")
            ax.plot(N_smooth, spline_t(N_smooth), color=cor_tmp, lw=2.5, alpha=0.9, linestyle="--", label="Tempo real (normalizado)")
        else:
            ax.plot(Ns_arr, O_norm, color=cor_ops, lw=2.5, alpha=0.9, label="Operacoes (normalizado)")
            ax.plot(Ns_arr, T_norm, color=cor_tmp, lw=2.5, alpha=0.9, linestyle="--", label="Tempo real (normalizado)")

        ax.scatter(Ns_arr, O_norm, color=cor_ops, s=110, zorder=10, edgecolors="#FFFFFF", linewidths=1.5, marker="o")
        ax.scatter(Ns_arr, T_norm, color=cor_tmp, s=110, zorder=10, edgecolors="#FFFFFF", linewidths=1.5, marker="s")

        ax.set_title(f"Comparacao Normalizada: {nome_alg}",
                     color="#F0F0F0", fontsize=15, fontweight="bold", pad=15)
        ax.set_xlabel("Tamanho da lista (N)", color="#AAAAAA", fontsize=12)
        ax.set_ylabel("Valor normalizado (0-1)", color="#AAAAAA", fontsize=12)
        ax.tick_params(colors="#AAAAAA")
        for spine in ax.spines.values():
            spine.set_edgecolor("#333355")
        ...
        output_path = os.path.join(output_dir, f"comparacao_normalizada_{nome_alg.lower().replace(' ', '_')}.png")
        plt.savefig(output_path, dpi=160, bbox_inches="tight", facecolor=fig.get_facecolor())
        plt.close()
        print(f"Grafico salvo em {output_path}")